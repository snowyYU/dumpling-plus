...
export const validate: any = (args: object) => ({
  ...
  setup() {
    ...
    const onCheck = () => {
      LineEditTable.value?.validate((isValid: boolean, invalidFields?: ValidateFieldsError )=>{
...
// 下拉框select options
export const select_options: any = (args: object) => ({
  components: { DpLineEditTable },
  setup() {
    const columns = [
      {
...
select_options.storyName = "下拉框select options";

// 校验规则
export const rules: any = (args: object) => ({
  components: { DpLineEditTable },
  setup() {
    const LineEditTable = ref()
    const columns = [
        ...
        input_length_rules: [
          {
            validator: (rule: any, value: any, callback: any) => {
              return value?.length >= 6 ?callback() :callback(new Error('字符串长度必须>=6'))
            }
          }
        ]
      }
